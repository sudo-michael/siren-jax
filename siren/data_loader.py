...
class BaseImageLoader(ABC):
    ...
    def create_batches(self):
        if not self.do_batch:
            ...
            self.batched_x, self.num_batches = split_to_batches(
                self.x, size=self.batch_size
            )
            self.batched_y, self.num_batches = split_to_batches(
                self.y, size=self.batch_size
            )
...
class CombinedImageLoader:
    ...
    def create_batches(self):
        ...
        if self.do_batch:
            x, y_vani, y_grad, y_lapl = shuffle_arrays_in_same_order(
                [x, y_vani, y_grad, y_lapl]
            )
